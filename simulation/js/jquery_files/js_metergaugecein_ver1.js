...
function secondres(){
	...
	ic = parseFloat(ib_int)*Math.sqrt(parseFloat(ib_int))*(Math.exp(parseFloat(vce))-Math.exp(-parseFloat(vce)))/(Math.exp(parseFloat(vce))+Math.exp(-parseFloat(vce))); //mili ampere
...
function check() {
 //initially base_res=1,collector_res=1
	...
	ib_int=2*Math.exp(parseFloat(vbe)/0.7); //micro ampere
	ib=(ic/bf)+2*Math.exp(parseFloat(vbe)/0.7);
