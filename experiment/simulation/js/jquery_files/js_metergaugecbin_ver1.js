...
function secondres(){
  ...
  vbc=collector_res*0.1;
     ...
     vcb=0.1*collector_res;
      ic =parseFloat(ie_int)*Math.sqrt(ie_int)* Math.exp((parseFloat(vcb) /0.7) - 1);
...
function check() {
     ...
     ie_int = 2 * Math.exp((parseFloat(vbe) /0.7) - 1).toPrecision(4); //in mili ampere 
	 ie= (ic/alpha)+2 * Math.exp((parseFloat(vbe) /0.7) - 1)
